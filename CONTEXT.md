...
**flash-site**:
The general public web presence for the project, published from `flash-site/`. Its home page (`index.html`) offers two paths: flashing a prebuilt firmware `.bin` over Web Serial via esp-web-tools (`flash.html`, the "visit a page, click a button" install experience), and configuring a new build (`configure.html`, the Variant picker + BOM + STL downloads — see Configurator). `docs/`'s BOM/workshop materials are now background/archival references rather than the authoritative parts list.
_Avoid_: "the website" (ambiguous with the on-device control UI served by the firmware itself). "The flashing site" alone, now that it's not flashing-only.
...
**Configurator**:
`flash-site/configure.html` and its `js/` modules — lets a prospective builder pick a Variant and see the resulting BOM and STL download links, before anything is built or flashed. Distinct from the Control UI (runs on an already-built, already-flashed boat) and from flashing (installs firmware onto already-built hardware).
...
> "No — flash-site is a separate, public site. Its flashing page installs the `.bin` the first time. The Control UI only exists once the firmware is already on the board."
>
> "Where do I find out what parts I need to build one?"
> "flash-site's Configurator — pick a Variant (bottle size, driver, motor, prop) and it resolves the BOM and STL downloads for that exact combination. The old per-workshop docs under `docs/materials/` are archival now, not the source of truth."
